...
def plot_ae_training_history(path, model_type):
    ...
    labels = [
        "training (Radiation Parameters)",
        "training (Outboard Plasma Profiles)",
        "training (Divertor Plasma Profiles)",
        "validation (Radiation Parameters)",
        "validation (Outboard Plasma Profiles)",
        "validation (Divertor Plasma Profiles)",
...
def plot_ae_validation_statistics(path):
    ...
    for i, ax in zip(range(9), axs.ravel(), strict=False):
        if i == 0:
...
def plot_lsr_distribution(path):
    ...
    for i, ax in zip(range(zdims), axs.ravel(), strict=False):
