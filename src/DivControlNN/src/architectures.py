...
def apply_layers_on(layers, x):
    ...
    assert all([isinstance(layer, keras.layers.Layer) for layer in layers]), (
...
def mm_prf1_cnn(input_dim, latent_dim):
    ...
    # Encoder definition
    encoder = [
        keras.layers.Input(shape=input_dim, name="input_prf1"),
    ...
    decoder = [
        keras.layers.Input(shape=(latent_dim,), name="zdec_prf1"),
        # keras.layers.BatchNormalization(),
...
def mm_prf2_cnn(input_dim, latent_dim):
    ...
    # Encoder definition
    encoder = [
        keras.layers.Input(shape=input_dim, name="input_prf2"),
    ...
    decoder = [
        keras.layers.Input(shape=(latent_dim,), name="zdec_prf2"),
        # keras.layers.BatchNormalization(),
        keras.layers.Dense(k[0] * k[1]),
        keras.layers.BatchNormalization(),
...
def multimodal_dc(input_shp, latent_dim, feat_sz, do_vae=True):
    ...
    """
    print(
        f"--- architectures.py:multimodal_dc() input_shp = {input_shp}, latent_dim = {latent_dim}"
        f" feat_sz = {feat_sz} do_vae = {do_vae}"
    )

    # Validate input shapes
    assert isinstance(input_shp, tuple) and len(input_shp) == 3, f"Invalid input shape: {input_shp}"
    dim_scal, dim_prf1, dim_prf2 = input_shp
    assert isinstance(dim_scal, tuple) and len(dim_scal) == 2, f"Invalid scalar size: {dim_scal}"
    assert isinstance(dim_prf1, tuple) and len(dim_prf1) == 2, f"Invalid profile size: {dim_prf1}"
    assert isinstance(dim_prf2, tuple) and len(dim_prf2) == 2, f"Invalid profile size: {dim_prf2}"
